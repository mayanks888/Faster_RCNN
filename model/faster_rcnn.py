...
class _Faster_RCNN_Maker(nn.Module):
    ...
    def loss(self, image, gt_bbox, gt_bbox_label):
        ...
        features = self.feature_extractor(image)#this is like a function of aclass which lets us run the vg16 model on input images
...
def faster_rcnn(n_class, backbone='vgg16', model_path=None):
    if backbone == 'vgg16':
        ...
        #head is a class of vgg16 head
        extractor, head, feature_dim = get_vgg16_extractor_and_head(n_class, roip_size=7, vgg_pretrained=vgg_pretrained)
        #rpn_net is also a class of rpn, remember this object oriented programing
        rpn_net = rpn(in_channel=feature_dim, mid_channel=512,ratio=[0.5, 1, 2], anchor_size=[128, 256, 512])
        #model is class created from the instance of _faster_RCNN_maker which took rpn_net class as parent
...
if __name__ == '__main__':
    ...
    image = np.random.rand(3,500,600)*255
