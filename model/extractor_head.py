...
def get_vgg16_extractor_and_head(n_class, roip_size=7, vgg_pretrained=False):
    ...
    features = list(vgg16_net.features)[0:30]#you can convert model into two parts feature here and classifier later in line 23
    
    for layer in features[0:10]:    # freeze top 4 conv2d layers(out of 10 5 are max pool layer so we tool 10)
        for p in layer.parameters():#this will actually show you the parameter(wieghts store in that saved model)
            p.requires_grad = False
    extractor = nn.Sequential(*features)#passing all arguments of features into sequential layers(its convert layer into sequential layer
    ...
    if torch.cuda.is_available():
        ...
        # Till Here we  have created  pretrainied model and convert them into sequential layer and remove last layer
    head = _VGG16Head(n_class_bg=n_class+1, roip_size=roip_size, classifier=classifier)#this is like a define a class head()
    if torch.cuda.is_available():
...
#dont forget to look for roip now
    ...
    def forward(self, feature_map, rois, image_size):
        ...
        # ie (600x800 of image scale to 37-57 of feature output scale)
    ...
    def predict(self, roi, delta_per_class, score, image_size, prob_threshold=0.5):
        ...
        if torch.cuda.is_available():
            ...
            delta_per_class = delta_per_class * torch.FloatTensor([0.1, 0.1, 0.2, 0.2]) + torch.FloatTensor([0., 0., 0., 0.])
